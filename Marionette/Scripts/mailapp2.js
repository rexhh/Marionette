BBCloneMail = (function(Backbone, Marionette){
  ...
  // called after all initializers are run
  ...
  // args passed in are from start, they are the regions
  App.startSubApp = function(appName, args){
    ...
    // call Marionette Module start, the args are passed to the module initializer callbacks
...
// used in the base Controller object

// SelectorView (itemView), Controller
BBCloneMail.AppSelector = (function(App, Marionette){
  ...
  SelectorView = Marionette.ItemView.extend({
    ...
    // when the combo box is changed, call function appSelected()
    events: {
      "change": "appSelected"
    },
    ...
    appSelected: function(e){
      ...
      // trigger event 'app:selected'
      // the controller listens for it
  ...
  return Marionette.Controller.extend({
    ...
    _getSelectorView: function(){
      ...
      // event 'render' is triggered after render()
...
// used by MailApp and ContactsApp
...
// CategoryListView (ItemView), Nav.Menu (Controller)
...
// Category (Model), CategoryCollection (Collection), Controller
BBCloneMail.module("MailApp.Categories", function(Categories, App, Backbone, Marionette, $, _){
  ...
  // addInitializer for this module
...
BBCloneMail.module("MailApp", function(MailApp, App){
  ...
  // addInitializer for this module, run when the module is started
...
// ContactView (ItemView), ContactListView (CollectionView), CategoryView (ItemView), Controller (Controller)
...
// Contact (Model), ContactCollection (Collection) , Repository (Controller)
...
// Backbone Models for Email (Model) and EmailCollection (Collection), and the Mail.Mailbox (Controller)
...
// MailView (ItemView) and MailViewer (Controller)
...
// MailPreview (ItemView), MailListView (CollectionView), Inbox (Controller)
BBCloneMail.module("MailApp.Mailboxes", function(Mailboxes, App, Backbone, Marionette, $, _){
  "use strict";
